...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='thana.conf'
CONFIGFOLDER='/root/.thana'
COIN_DAEMON='thanad'
COIN_CLI='thana-cli'
COIN_PATH='/usr/local/bin/'
COIN_REPO='https://github.com/thana-dev/thana-core.git'
COIN_TGZ='https://github.com/thana-dev/thana-core/releases/download/3.0.0/thana-3.0.0-x86_64-linux-signed.tar.gz'
...
function get_ip() {
  declare -a NODE_thana
  for thana in $(netstat -i | awk '!/Kernel|Iface|lo/ {print $1," "}')
  do
    NODE_thana+=($(curl --interface $thana --connect-timeout 2 -s4 icanhazip.com))
  done
  ...
  if [ ${#NODE_thana[@]} -gt 1 ]
    then
      ...
      INDEX=0
      for ip in "${NODE_thana[@]}"
      do
        echo ${INDEX} $ip
        let INDEX=${INDEX}+1
      done
      read -e choose_ip
      NODEIP=${NODE_thana[$choose_ip]}
  else
    NODEIP=${NODE_thana[0]}
  fi
}
