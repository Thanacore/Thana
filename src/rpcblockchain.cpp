...
Value getrawmempool(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in thana\n"
...
Value gettxout(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of thana addresses\n"
...
Value getchaintips(const Array& params, bool fHelp)
{
    ...
    /* Build up a list of chain tips.  We start with the list of all
       ...
       of another block.  */
    std::set<const CBlockIndex*, CompareBlocksByHeight> setTthana;
    BOOST_FOREACH (const PAIRTYPE(const uint256, CBlockIndex*) & item, mapBlockIndex)
        setTthana.insert(item.second);
    BOOST_FOREACH (const PAIRTYPE(const uint256, CBlockIndex*) & item, mapBlockIndex) {
        const CBlockIndex* pprev = item.second->pprev;
        if (pprev)
            setTthana.erase(pprev);
    }

    // Always report the currently active tip.
    setTthana.insert(chainActive.Tip());

    /* Construct the output array.  */
    Array res;
    BOOST_FOREACH (const CBlockIndex* block, setTthana) {
        Object obj;
